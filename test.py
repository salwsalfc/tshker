...
class Ze_GlobalCollection(BASE):
    __tablename__ = "ze_globalcollection"
    ...
    def __repr__(self):
        return "<Ze Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Ze_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Ze_GlobalCollection.__table__.create(checkfirst=True)

ZE_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with ZE_GLOBALCOLLECTION:
        keyword_items = Ze_GlobalCollection(keywoard, tuple(contents))
...
def del_keyword_collectionlist(keywoard):
    with ZE_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Ze_GlobalCollection.keywoard)
            .filter(Ze_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(
            Ze_GlobalCollection.keywoard).distinct().all()
